...
* Note
I've turned off some things to try get better performance with large org files:
- word-count in modeline
- org-roam completion everywhere
- The entire 'aesthetics' block (including org-superstar, mixed-pitch, etc.)

Result: improved performance a lot!! Next, will have to try turning things back on one by one, to see what was the culprit...
** TODO find what is slowing org mode
