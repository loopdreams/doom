...
** TODO sending mail is broken, not sure what to do with it at the moment. I can send from other clients (mutt) which use the same msmtp settings, but not from here...
#+BEGIN_SRC emacs-lisp
(setq
